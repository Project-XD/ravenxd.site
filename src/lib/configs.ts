...
export const getConfigById = (id: number) => configs.find(c => c.id === id);

export const searchConfigs = (query: string) => {
    if (!query) return configs;
    const q = query.toLowerCase();
    return configs.filter(config =>
        config.name.toLowerCase().includes(q) ||
        config.creator.toLowerCase().includes(q) ||
        config.ip.toLowerCase().includes(q) ||
        config.tags.some(tag => tag.toLowerCase().includes(q)) ||
        (Array.isArray(config.anticheat)
            ? config.anticheat.some(ac => ac.toLowerCase().includes(q))
            : config.anticheat.toLowerCase().includes(q))
    );
};
